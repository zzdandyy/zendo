...
- **Dual-pane cross transfer**: `cross_transfer` Tauri command streams file contents from source to destination via 64KB chunks, no local disk. Supports SFTP, SCP, and S3 (local↔s3). Walks directory trees for folder transfers, emits real progress (total_bytes, bytes_transferred, speed, ETA). Validates local paths with `validate_local_path`.
- **Cross-transfer → TransferStore unification**: `use-sftp-transfers.ts` listens on `cross:transfer` channel and normalizes `CrossTransferEvent` → `TransferEvent` with sentinel `sftp_session_id: "__cross__"`. This makes cross-pane transfers visible in the TransferPopover and FAB, sharing cancel/dismiss with queue-based transfers.
- **Upload auto-refresh (polling)**: `Pane.tsx` listens for transfer events per transport channel, but does NOT react to every progress event (too frequent). Instead: when any upload enters InProgress/Queued, start a 3 s polling loop (first refresh at 300 ms). Stop polling when all active uploads complete. Completion also triggers an immediate refresh.
...
- **New File/Folder in context menu, not toolbar**: ExplorerToolbar no longer has New File / New Folder icon buttons. Those actions live on the right-click context menu of any entry (file, folder, or empty area) and on the empty-folder placeholder buttons. `ExplorerFileTable` receives `onStartCreateFile` / `onStartCreateFolder` props that toggle the inline create row via `useState` in the parent.
- **ContextMenu uses `createPortal`**: ContextMenu renders into `document.body` to escape `overflow-hidden` ancestors. Submenus use a 150 ms `setTimeout` on `mouseLeave` (cleared on `mouseEnter`) to prevent disappearing when the cursor moves from the parent item to the submenu.
- **Drag-drop pane isolation**: Tauri's `onDragDropEvent` fires globally (all webview windows receive it). `isOverPane()` checks DOM bounds (`getBoundingClientRect`) of the pane ref against the event position. `enter`/`over`/`drop` handlers only respond when the cursor is inside their pane.
